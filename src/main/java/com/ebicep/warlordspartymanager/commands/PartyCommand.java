...
public class PartyCommand implements TabExecutor {
            ...
            case "p":
                ...
                switch (input.toLowerCase()) {
                    case "invite": {
                        ...
                        if (WarlordsPartyManager.inSameParty(player.getUniqueId(), invitedPlayer.getUniqueId())) {
                    ...
                    case "join": {
                        ...
                        Optional<Party> party = WarlordsPartyManager.getPartyFromLeader(partyLeader.getUniqueId());
                        if (!party.isPresent()) {
                            ChatUtils.sendMessageToPlayer(player, ChatColor.RED + "That player does not have a party!", ChatColor.BLUE, true);
                            return true;
                        }
                    ...
                    case "forcejoin": {
                        currentParty.ifPresent(party -> {
                            if (sender.hasPermission("warlords.party.forcejoin")) {
                                ...
                                if (WarlordsPartyManager.inSameParty(player.getUniqueId(), playerToForceInvite.getUniqueId())) {
                                    ChatUtils.sendMessageToPlayer(player, ChatColor.RED + "That player is already in the party!", ChatColor.BLUE, true);
                                    return;
                                }
    ...
    public List<String> onTabComplete(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length > 1) {
            ...
            WarlordsPartyManager.getPartyFromAny(((Player) commandSender).getUniqueId()).ifPresent(party -> party.getPartyPlayers().stream()
